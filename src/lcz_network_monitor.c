...
#if defined(CONFIG_DNS_RESOLVER)
	dns_addr = &(net_sin6(&dns->servers[0].dns_server)->sin6_addr);
#endif
	net_addr = &cfg->ip.ipv6->unicast->address.in6_addr;
	if (dns_addr && !net_ipv6_is_addr_unspecified(dns_addr)) {
		update_string(AF_INET6, dns_addr, ATTR_ID_ipv6_addr, "DNS IPV6");
	} else if (!net_ipv6_is_addr_unspecified(net_addr)) {
...
#if defined(CONFIG_DNS_RESOLVER)
	dns_addr = &(net_sin(&dns->servers[0].dns_server)->sin_addr);
#endif
	net_addr = &cfg->ip.ipv4->unicast->address.in_addr;
	if (dns_addr && !net_ipv4_is_addr_unspecified(dns_addr)) {
		update_string(AF_INET, dns_addr, ATTR_ID_ipv4_addr, "DNS IPV4");
	} else if (!net_ipv4_is_addr_unspecified(net_addr)) {
